...
async def call_gpt4o_async(prompt, row_index):
    """Async wrapper for GPT-4o API calls"""
    try:
...
def batch_call_gemma3(prompts, row_indices):
    """Batch process multiple prompts with Gemma3"""
    try:
        model = get_gemma_model()
        chat_messages_list = [
        ...
        responses = model.batch_chat(chat_messages_list)

        results = []
        for i, response in enumerate(responses):
            try:
                # Handle response format from batch_chat (based on tutorial)
                content = response.outputs[0].text
                results.append(content)
            except Exception as e:
                print(f" Failed to extract response for row {row_indices[i] + 1}: {e}")
                results.append(f"ERROR: {e}")
        return results
    except Exception as e:
        print(f" Batch model call failed: {e}")
        return [f"ERROR: {e}" for _ in prompts]


async def batch_call_gpt4o(prompts, row_indices, batch_size=5):
    """Batch process multiple prompts with GPT-4o using async"""
    results = []
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Batch size for parallel processing (default: 10 for gemma3, 5 for gpt4o).",
    )
    ...
    batch_size = (
        args.batch_size if args.model_type == "gemma3" else min(args.batch_size, 5)
    )
    ...
    unprocessed_indices = [i for i in range(len(df)) if not processed_mask.iloc[i]]
        ...
        for i in range(0, len(unprocessed_indices), batch_size):
            ...
            try:
                ...
                if args.model_type == "gemma3":
                    model_outputs = batch_call_gemma3(prompts, batch_indices)
                else:
                    model_outputs = await batch_call_gpt4o(
                        prompts, batch_indices, batch_size
                    )

                # Process results
                for j, (idx, output) in enumerate(zip(batch_indices, model_outputs)):
                    score, justification = extract_score_and_justification(output)
                ...
                print(
                    f"⚠️ Error processing batch starting at row {batch_indices[0] + 1}: {str(e)[:100]}..."
                )
            ...
            if processed_counter % (batch_size * 2) == 0:
                ...
                print(
                    f"💾 Autosaved after processing {processed_counter} rows → {checkpoint_path} (Total processed: {current_processed}/{total_rows})"
                )

    # Run the batch processing
    if args.model_type == "gpt4o":
        asyncio.run(process_batches())
    else:
        # For gemma3, we don't need async
        processed_counter = 0
    ...
    processed_rows = len(unprocessed_indices)

    print(f"\n🎉 Final output saved to: {output_path}")
    print(f"⏱️ Processing completed in {elapsed_time:.2f} seconds")
    print(
        f"📊 Processed {processed_rows} rows at {processed_rows / elapsed_time:.2f} rows/second"
    )
    print(
        f"🚀 Batch size: {batch_size} - Speed improvement: ~{batch_size}x over sequential processing"
    )
